...
const { authenticate, ownsBookmark, isGroupMember } = require("../middleware");

// api/groups/:groupId/books/:bookId/bookmarks

router.route("/")
    .get(authenticate, catchAsync(isGroupMember), catchAsync(index))
    .post(authenticate, catchAsync(isGroupMember), catchAsync(addBookmark))

router.route("/:bookmarkId")
    .put(authenticate, catchAsync(isGroupMember), catchAsync(ownsBookmark), catchAsync(updateBookmark))
